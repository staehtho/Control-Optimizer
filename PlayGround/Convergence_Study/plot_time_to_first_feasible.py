...
DATA_PATH = (
    ...
    / "curve_time_to_first_feasible.csv"
)
...
def build_series(rows: list[dict[str, str]]) -> dict[tuple[str, str], list[tuple[float, float]]]:
    ...
    for key, group_rows in grouped.items():
        ...
        for row in ordered_rows:
            ...
            first_feasible_iteration = float(row["time_to_first_feasible"])
            points.append((run_rank, first_feasible_iteration))
        series[key] = points
    return series
...
def main() -> None:
    ...
    ax.set_xlabel("Laufindex nach ansteigender Zeit bis zur ersten zulaessigen Loesung")
    ax.set_ylabel("Iteration der ersten zulaessigen Loesung")
    ax.set_title("Zeit bis zur ersten zulaessigen Loesung")
