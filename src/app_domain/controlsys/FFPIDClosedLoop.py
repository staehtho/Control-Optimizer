...
# Description:   Implements the FFPIDClosedLoop class, providing a closed-loop control system
#                based on an ideal/ISA Feed-Forward-PID controller parameterized either in parallel gain form
...
class FFPIDClosedLoop(ClosedLoop):
    ...
    def __init__(self,
        ...
        """
        Closed-loop control system using an ideal/ISA FF-PID controller.

        The feedback part is a standard ideal/ISA PID controller. In addition,
        the controller family includes a static setpoint feedforward term that
        is added directly to the unsaturated control signal before applying the
        control constraint:

            u(t) = u_PID(t) + Kff * r(t)

        where ``r(t)`` is the setpoint and ``Kff`` is a constant feedforward
        gain. The resulting sum is then passed through the actuator saturation
        or control-output limitation.

        The PID feedback part can be parameterized using either **parallel gain
        form** or **ISA time-constant form**:
        ...
        Only one PID parameterization method should be provided during
        initialization. If one form is provided, the parameters of the other
        form are computed automatically.

        The derivative part of the PID feedback path is filtered using a
        first-order (PT1) filter with time constant `Tf`.

        Implemented PID feedback transfer function (ideal/ISA form):
            Gc_pid(s) = Kp * (1 + 1/(Ti * s) + (Td * s) / (Tf * s + 1))

        Overall FF-PID control law:
            U(s) = Gc_pid(s) * E(s) + Kff * R(s)
        ...
        Args:
            ...
            Kp (float, optional): Proportional gain of the PID feedback path
                (parallel gain form).
            Ki (float, optional): Integral gain of the PID feedback path
                (parallel gain form).
            Kd (float, optional): Derivative gain of the PID feedback path
                (parallel gain form).

            Ti (float, optional): Integral time constant of the PID feedback
                path (ISA time-constant form).
            Td (float, optional): Derivative time constant of the PID feedback
                path (ISA time-constant form).

            Tf (float, optional): Time constant of the derivative PT1 filter in
                the PID feedback path. Defaults to 0.01.
        ...
        Raises:
            ValueError: If both PID parameterization methods (parallel gain and
                ISA time-constant form) are provided or if neither is provided.
        """
